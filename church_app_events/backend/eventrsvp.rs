        ...
        Ok(rsvps) => {
            let status_counts = HashMap::from([
                ("confirmed".to_string(),
                    rsvps.iter()
                    .filter(|r: &&PgRow|
                        r.get::<RsvpStatus, _>("rsvp_status") == RsvpStatus::Confirmed)
                    .count()),
                ("pending".to_string(),
                    rsvps.iter()
                    .filter(|r: &&PgRow|
                        r.get::<RsvpStatus, _>("rsvp_status") == RsvpStatus::Pending)
                    .count()),
                ("declined".to_string(),
                    rsvps.iter()
                    .filter(|r: &&PgRow|
                        r.get::<RsvpStatus, _>("rsvp_status") == RsvpStatus::Declined)
                    .count()),
            ]);
